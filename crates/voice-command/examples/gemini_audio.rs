...
fn decode_mp3(path: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        sample_buffer.copy_interleaved_ref(
            decoded,
        );
...
fn write_pcm_wav(
    ...
    if pcm.len() % 2 != 0 {
        return Err(
            "PCM byte count must be even for 16-bit audio".into()
        );
    }
    ...
    -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let input_path =
        env::args()
            .nth(1)
            .ok_or(
                "usage: cargo run -p voice-command --example gemini_audio -- <mp3-file>",
            )?;
    ...
    let audio =
        decode_mp3(&input_path)?;

    println!(
        "Decoded {} bytes of 16 kHz mono PCM.",
        audio.len()
    );
    ...
    println!(
        "✓ Wrote {} bytes to gemini-output.wav",
        output.len()
    );
