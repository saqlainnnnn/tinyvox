...
use tinyvox_engine::{
    ...
    tool_registry::ToolRegistry, tools::ToolRequest,
};

use voice_command::{
    GeminiLiveProvider, VoiceEvent, VoiceProvider,
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...

    {
        ...
        last.replace("Hello from the TinyVox tool test");
    }
    ...
    let receiver = tokio::spawn(receive_loop(send_handle, receive_handle, registry));
    ...
    let send_handle = GeminiLiveProvider::from_env()?;
...
async fn receive_loop(
    ...
    send_handle
        .send_text("What was my last dictation? Use the read_last_dictation tool to find out.")
        .await?;
    ...
    loop {
        ...
        .await
        .map_err(|_| "timed out waiting for Gemini")??;

        match event {
            VoiceEvent::ToolCall(tool_call) => {
                println!("🔧 Tool requested: {}", tool_call.name,);
                ...
                let response = serde_json::to_value(&result).unwrap_or_else(|_| {
                    json!({
                        "error":
            ...
            VoiceEvent::TurnComplete => {
                println!("✓ Gemini finished the turn.");
