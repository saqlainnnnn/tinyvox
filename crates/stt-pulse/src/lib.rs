    ...
    fn fmt(
        ...
        match self {
            ...
            Self::InvalidUrl(error) => {
                write!(
                    f,
                    "failed to construct Pulse API URL: {error}"
                )
            }
            ...
            Self::Http(error) => {
                write!(
                    f,
                    "Pulse HTTP request failed: {error}"
                )
            }
            ...
            Self::Api { status, body } => {
                write!(
                    f,
                    "Pulse API returned {status}: {body}"
                )
            }
            ...
            Self::InvalidResponse(error) => {
                write!(
                    f,
                    "failed to decode Pulse response: {error}"
                )
            }
        }
    }
...
impl PulseClient {
    pub fn from_env() -> Result<Self, PulseError> {
        let api_key = env::var("SMALLEST_API_KEY")
            .map_err(|_| PulseError::MissingApiKey)?;
...
impl SpeechToText for PulseClient {
    ...
    fn transcribe(
        ...
        Output = Result<Transcript, Self::Error>,
    > + Send {
...
fn pcm16_bytes(audio: &AudioBuffer) -> Vec<u8> {
    let mut bytes =
        Vec::with_capacity(audio.samples.len() * 2);
