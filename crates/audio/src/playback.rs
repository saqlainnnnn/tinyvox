    ...
    fn fmt(
        ...
        match self {
            ...
            Self::BufferPoisoned => {
                write!(
                    f,
                    "playback buffer lock was poisoned"
                )
            }
...
impl CpalAudioPlayback {
    ...
    pub fn start(
        ...
        let error_callback =
            |error| {
                eprintln!(
                    "TinyVox playback stream error: {error}"
                );
            };
    ...
    pub fn push_pcm16(
        ...
        queue.extend(
            resampled.iter().copied(),
        );

        Ok(())
    }

    pub fn queued_samples(
        &self,
    ) -> Result<usize, AudioPlaybackError> {
        let queue =
            self.queue
                .lock()
                .map_err(|_| {
