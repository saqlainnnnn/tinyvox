...
impl Dictionary {
    ...
    pub(crate) fn from_entries(entries: Vec<DictionaryEntry>, next_id: u64) -> Self {
    ...
    pub fn apply(&mut self, transcript: &str) -> String {
        ...
        for entry in &mut self.entries {
            let replaced = replace_tokens(&result, &entry.wrong, &entry.correct);
    ...
    pub fn add(&mut self, wrong: &str, correct: &str, source: EntrySource) -> EntryId {
    ...
    pub fn edit(&mut self, id: EntryId, wrong: &str, correct: &str) -> bool {
...
fn replace_tokens(text: &str, wrong: &str, correct: &str) -> String {
    if wrong.is_empty() {
        return text.to_string();
    }

    text.split_inclusive(|character: char| {
    ...
    .map(|token| {
        ...
        while boundary > 0 {
            let character = token[..boundary].chars().next_back();

            if character.is_some_and(|c| c.is_whitespace() || c.is_ascii_punctuation()) {
                boundary -= character.unwrap().len_utf8();
            } else {
                break;
            }
        }
...
mod tests {
    ...
    fn adds_entry() {
        ...
        let id = dictionary.add("saqlain", "Saqlain", EntrySource::Manual);
    ...
    fn replaces_exact_token() {
        ...
        assert_eq!(
            dictionary.apply("I use Kubernets every day."),
    ...
    fn matching_is_case_insensitive() {
        ...
        dictionary.add("saqlain", "Saqlain", EntrySource::Manual);

        assert_eq!(dictionary.apply("SAQLAIN is here."), "Saqlain is here.");
    }
    ...
    fn punctuation_is_preserved() {
        ...
        assert_eq!(
            dictionary.apply("Kubernets, Kubernets!"),
    ...
    fn does_not_replace_inside_words() {
        ...
        dictionary.add("cat", "dog", EntrySource::Manual);

        assert_eq!(
            dictionary.apply("concatenate cat bobcat"),
    ...
    fn multiple_entries_are_applied() {
        ...
        assert_eq!(
            dictionary.apply("Kubernets by Saqlain"),
    ...
    fn hit_count_increments() {
        ...
        dictionary.apply("hello saqlain");

        assert_eq!(dictionary.entries()[0].hit_count, 1);

        dictionary.apply("hello saqlain again");

        assert_eq!(dictionary.entries()[0].hit_count, 2);
    }
    ...
    fn removing_unknown_entry_returns_false() {
        ...
        assert!(!dictionary.remove(EntryId(999)));
    }
    ...
    fn unchanged_text_is_returned() {
        ...
        assert_eq!(dictionary.apply("hello from TinyVox"), "hello from TinyVox");
    }
}
