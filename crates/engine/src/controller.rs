use crate::{
    dictionary::{SharedDictionary, shared as shared_dictionary},
    event::AppEvent,
    last_dictation::{SharedLastDictation, shared},
    ports::{AudioRecorder, CleanedText, SpeechToText, TextCleaner, TextInjector, Transcript},
    state::AppState,
...
pub enum ControllerError<RE, SE, CE, IE> {
    InvalidTransition { state: AppState, event: AppEvent },
    Recorder(RE),
    SpeechToText(SE),
    Cleaner(CE),
    Injector(IE),
...
impl<RE: std::fmt::Display, SE: std::fmt::Display, CE: std::fmt::Display, IE: std::fmt::Display>
    std::fmt::Display for ControllerError<RE, SE, CE, IE>
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidTransition { state, event } => {
                write!(f, "invalid transition from {state:?} using {event:?}")
            }
...
impl<RE, SE, CE, IE> std::error::Error for ControllerError<RE, SE, CE, IE>
where
    ...
    I: TextInjector,
{
    ...
    //
    // Keeps the current API working for existing callers/tests.
    // Creates an isolated LastDictation when no shared handle
    // is supplied.
    pub fn new(
        recorder: R,
        speech_to_text: S,
    ...
    // Constructor for the real application where the controller
    // and ToolRegistry must share the same LastDictation state.
    pub fn new_with_last_dictation(
        recorder: R,
        speech_to_text: S,
        dictionary: SharedDictionary,
        cleaner: C,
    ...
    pub fn start_recording(
        &mut self,
    ) -> Result<(), ControllerError<R::Error, S::Error, C::Error, I::Error>> {
        ...
        self.injector.prepare().map_err(ControllerError::Injector)?;

        self.recorder.start().map_err(ControllerError::Recorder)?;
        ...
        F: Fn(AppState),
    {
        ...
        let audio = self.recorder.stop().map_err(ControllerError::Recorder)?;

        let recording_ms = if audio.sample_rate == 0 {
            0
        } else {
        ...
        let corrected_text = {
            ...
            dictionary.apply(&transcript.text)
        };
        ...
        // ---------------------------------------------------------
        // Successful injection → LastDictation
        // ---------------------------------------------------------

        {
            let mut last_dictation = self.last_dictation.write().unwrap();

            last_dictation.replace(cleaned_text.text.clone());
        }

        // ---------------------------------------------------------
        // Record stats only after successful injection
        // ---------------------------------------------------------

        let word_count = cleaned_text.text.split_whitespace().count() as u32;

        self.stats.record(day, word_count, recording_ms);
...
mod tests {
    ...
    use crate::{
        dictionary::{EntrySource, shared as shared_dictionary},
        last_dictation::shared as shared_last_dictation,
        ports::{AudioBuffer, CleanedText, Transcript},
    };
    ...
    impl SpeechToText for FakeSpeechToText {
        ...
        async fn transcribe(&self, audio: &AudioBuffer) -> Result<Transcript, Self::Error> {
    ...
    impl SpeechToText for DictionarySpeechToText {
        ...
        async fn transcribe(&self, _audio: &AudioBuffer) -> Result<Transcript, Self::Error> {
            Ok(Transcript {
                text: "I use Kubernets every day.".to_string(),
            })
        }
    }
    ...
    impl TextCleaner for FakeCleaner {
        ...
        fn clean(
            ...
            async move {
                *received.lock().unwrap() = Some(text.clone());
    ...
    impl TextInjector for FakeInjector {
        ...
        fn inject(&self, _text: &CleanedText) -> Result<(), Self::Error> {
            Ok(())
        }
    }
    ...
    async fn recording_lifecycle_is_managed_by_controller() {
        let recorder = FakeRecorder { recording: false };
        ...
        controller.stop_recording(test_day(), |_| {}).await.unwrap();
    ...
    async fn successful_injection_records_stats() {
        ...
        assert_eq!(controller.stats().total_dictations(), 1,);

        assert_eq!(controller.stats().total_words(), 3,);

        assert_eq!(controller.stats().today(test_day(),).dictations, 1,);

        assert_eq!(controller.stats().today(test_day(),).words, 3,);
    }
    ...
    async fn successful_injection_updates_last_dictation() {
        ...
        let last = last_dictation.read().unwrap();

        assert_eq!(last.text(), "hello from TinyVox",);
    }

    #[tokio::test]
    async fn shared_last_dictation_is_used_by_controller() {
        let last_dictation = shared_last_dictation();

        let mut controller = TinyVoxController::new_with_last_dictation(
            FakeRecorder { recording: false },
            FakeSpeechToText,
            shared_dictionary(),
            FakeCleaner {
                received: Arc::new(Mutex::new(None)),
